...
<div class="space-y-10 lg:space-y-0 lg:grid lg:grid-cols-2 lg:gap-12 lg:items-start">
  <!-- Left Side: Identity & Tables -->
  <div class="space-y-10">
      ...
      <div class="bg-gradient-to-r from-emerald-500 to-teal-500 p-6 rounded-[2rem] text-white shadow-xl shadow-emerald-500/30 animate-pulse">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <Download size={24} />
            <div>
              <h4 class="font-black text-lg">Update!</h4>
              <p class="text-sm opacity-90">New version ready.</p>
            </div>
          </div>
          <button 
            onclick={handleUpdate}
            class="px-4 py-2 bg-white text-emerald-600 font-black rounded-xl hover:bg-emerald-50 transition-all shadow-lg"
          >
            Go!
          </button>
        </div>
    ...
    <section class="space-y-6">
      ...
      <div class="grid grid-cols-4 sm:grid-cols-7 lg:grid-cols-5 gap-3">
  ...
  <div class="space-y-8">
    ...
    <div class="bg-indigo-600 p-8 rounded-[2.5rem] text-white shadow-xl shadow-indigo-600/30 relative overflow-hidden group">
      ...
      <p class="text-sm font-bold opacity-90 relative z-10 leading-relaxed">
        Choose 1 or 2 tables to practice at a time. It's the fastest way to get them to "Mastered" status!
      </p>
    </div>
      ...
      {#if showResetConfirm}
        <div class="bg-red-50 dark:bg-red-900/30 border-2 border-red-200 dark:border-red-800 p-6 rounded-[2rem] space-y-4">
          <div class="flex items-start gap-3">
            ...
            <div>
              <h4 class="font-black text-red-700 dark:text-red-400 text-sm">Are you sure?</h4>
              <p class="text-[10px] sm:text-xs text-red-600 dark:text-red-300 font-bold">
                Deletes all progress permanently. Cannot be undone.
              </p>
            </div>
          </div>
          <div class="flex gap-2">
            <button 
              ...
              class="flex-1 py-3 text-xs bg-red-600 text-white font-black rounded-2xl hover:bg-red-700 transition-all disabled:opacity-50"
            >
              Reset
            </button>
            <button 
              ...
              disabled={isResetting}
              class="flex-1 py-3 text-xs bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-black rounded-2xl hover:bg-slate-300 dark:hover:bg-slate-600 transition-all disabled:opacity-50"
            >
              Cancel
            </button>
          </div>
